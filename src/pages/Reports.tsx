...
import { FileBarChart, Users, DollarSign, Loader2, Star, TrendingUp, Download, MessageCircle, FileText } from "lucide-react";
...
export default function Reports() {
  ...
  const buildReportText = (periodLabel: string) => {
    let text = `📊 تقرير ${periodLabel} - د. خالد جادالله\n`;
  ...
  const downloadPDF = (periodLabel: string) => {
    const doc = new jsPDF({ putOnlyUsedFonts: true });
    
    const dateStr = new Date().toLocaleDateString("en-US");
    
    doc.setFontSize(18);
    doc.text(`د. خالد جادالله - ${periodLabel} Report`, 105, 20, { align: "center" });
    doc.setFontSize(11);
    ...
    const tableData = [
      ["Today Patients", String(stats.todayCount)],
      ["Month Patients", String(stats.monthCount)],
      ["Completed Visits", String(stats.completedCount)],
      ["Monthly Revenue", `${stats.monthlyRevenue.toLocaleString()} EGP`],
      ["Expenses", `${stats.totalExpenses.toLocaleString()} EGP`],
      ["Net Profit", `${stats.profit.toLocaleString()} EGP`],
      ["Avg Rating", `${stats.avgRating} (${stats.ratingsCount} ratings)`],
    ...
    if (stats.visitTypeData.length > 0) {
      doc.setFontSize(13);
      doc.text("Visit Types", 105, finalY + 12, { align: "center" });
      
      (doc as any).autoTable({
        startY: finalY + 17,
        head: [["Type", "Count"]],
        body: stats.visitTypeData.map(v => [v.name, String(v.value)]),
        theme: "grid",
    ...
    doc.text("Generated by د. خالد جادالله", 105, 285, { align: "center" });

    doc.save(`report-${periodLabel}-${new Date().toISOString().split("T")[0]}.pdf`);
    toast.success(`تم تحميل التقرير ${periodLabel} كـ PDF`);
  };
